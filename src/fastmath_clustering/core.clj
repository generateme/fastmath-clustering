(ns fastmath-clustering.core
  ...
  * `:size` - size of cluster"
  (:require [fastmath-clustering.distance :as d])
...
(defn- add-vectors [v1 v2] (map + v1 v2))

(defn- average-vectors
  [vss]
  (let [c (count vss)]
    (if (sequential? (first vss))
      (->> vss
           (reduce add-vectors)
           (map (fn [^double v] (/ v c))))
      (/ (reduce + vss) c))))
...
(defn- centroid-data
  ...
  {:representatives (seq (map seq (.centroids in)))
...
(defmacro ^:private clustering
  ...
  (let [[clss data-fn fit missing-predict?] (clustering-classes clustering-method)
    ...
    `(let [~obj (. ~clss ~(or fit 'fit) (d/seq->double-double-array ~data) ~@params)]
       (create-clusters ~clustering-method ~data ~obj ~data-fn
                        ~(if missing-predict?
                           `nil
                           `(fn [in#] (.predict ~obj (d/seq->double-array in#))))))))
...
(defn clarans
  ...
  * max-neighbor (optional) - maximum number of neighbors checked during random search
  ...
  ([data dist ^long clusters] (clarans data dist clusters (max 1 (Math/round (* 0.0125 clusters (- (count data) clusters))))))
